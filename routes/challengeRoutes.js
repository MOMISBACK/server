...
const { validateCreateChallenge, validateUpdateChallenge } = require('../validators/challengeValidators');
...
router.post('/', validateCreateChallenge, async (req, res) => { // ⚠️ Validation ajoutée
  try {
    const challenge = await challengeService.createChallenge(req.user._id, req.body);
...
router.put('/', validateUpdateChallenge, async (req, res) => { // ⚠️ Validation ajoutée
  try {
    const challenge = await challengeService.updateChallenge(req.user._id, req.body);
    res.json(challenge);
  } catch (error) {
